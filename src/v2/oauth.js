...
async function getAccessToken() {
  ...
  await pup.goTo(page, authorizationURL + "?" + params, {
  ...
  if (/checkpoint\/challenge/.test(pageUrl))
    throw new Error("Google recaptcha asked.");
  ...
  res["expiration_date"] = new Date().setSeconds(
    new Date().getSeconds() + res["expires_in"] - 3600 * 24 * 5
  ); // 5 days before the actual expiration date
