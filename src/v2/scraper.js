...
async function init() {
  try {
    console.info("Initializing the Linkedin client...");
    const accessToken = await oauth.getAccessToken();
    tokenExpirationDate = new Date().addSeconds(accessToken["expires_in"]);
    linkedin = nodeLinkedin(
      config.linkedinApiKey,
      config.linkedinApiSecret
    ).init(accessToken["access_token"]);
  } catch (e) {
    console.error(e);
...
async function getCompanyOrPeopleDetails(linkedinUrl, options = {}) {
  ...
  console.info('Getting data from "' + linkedinUrl + '"...');
  let browser = null;
  ...
  if (!isCompanyOrSchoolPage(linkedinUrl)) {
    if (options.forcePeopleScraping) {
      ...
      if (peopleDetails["message"]) {
        linkedinApiInternalError =
          peopleDetails["message"] == "Internal API server error";
        if (!linkedinApiInternalError)
          throw new Error(peopleDetails["message"]); // the linkedin URL is invalid
      }
    ...
    const companyId =
      peopleDetails["positions"]["values"] &&
      peopleDetails["positions"]["values"][0]["company"]["id"];
    if (companyId)
      linkedinUrl = "https://www.linkedin.com/company/" + companyId;
    else
      linkedinUrl =
        peopleDetails["currentCompany"] &&
        peopleDetails["currentCompany"]["linkedinUrl"];
...
async function scrapPeopleProfile(page, url = null) {
  if (url) {
    ...
    await logIn(page, config.linkedinEmail, config.linkedinPassword, {
      redirectionUrl: url
    });
  }
  ...
  const peopleDetails = await page.evaluate(() => {
    ...
    const experiences = $("#experience-section li").get().map(elt => {
      ...
      return {
        ...
        linkedinUrl:
          "https://www.linkedin.com" + elt.find("a.ember-view").attr("href")
      };
    });
      ...
      .map(elt => {
        ...
        return {
          ...
          linkedinUrl:
            ...
            elt.find("a.pv-browsemap-section__member").attr("href")
        };
      });
    return {
      firstName: name[0],
      ...
      school:
        $("a.pv-top-card-v2-section__link-education span").text().trim() ||
        null,
      connectionsNumber: parseInt(
        $("span.pv-top-card-v2-section__connections").text().match(/[0-9]+/)[0]
      ),
  ...
  const companyDetails = await page.evaluate(() => {
    ...
    for (let i = 0; i < keys.length; ++i) {
      ...
      if (key == "Website") companyDetails["website"] = value;
        ...
        companyDetails["foundedYear"] = parseInt(value);
