...
function createApp() {
  ...
  app.get("/request", saveIpAddress, async (req, res) => {
    console.info(
      `Request received from IP address = ${req.ipAddress} with linkedin URL = ${req
        .query.linkedinUrl}`
    );
    ...
    try {
      console.info(`Sending request to ${req.query.linkedinUrl}...`);
      const result = await linkedinClient.fetch(req.query.linkedinUrl);

      if (!result)
        res.json({
          error: "The people/company has not been found.",
          result: null
        });
      else res.json({ error: null, result });
...
if (process.env.NODE_ENV === "test") module.exports = { createApp };
else {
  (async () => {
    const app = createApp();
    app.listen(serverPort);
    console.info(
      `Server started on "http://localhost:${serverPort}", waiting for requests...`
    );
  })();
}
...
function saveIpAddress(req, res, next) {
  ...
  console.info("Number of shots for this address : " + ipAddresses[ipAddress]);
  if (ipAddresses[ipAddress] > maximumShotsNumber)
    res.json({
      error:
        "You have reached your maximum number of trials, contact me if you wish to work with me."
    });
  else {
